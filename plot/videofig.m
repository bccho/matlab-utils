...
%     Spacebar -- play/pause video (30 frames-per-second default).
...
%   Edited by Byung-Cheol Cho, 2016
    
    %default parameter values
    if nargin < 3 || isempty(play_fps), play_fps = 30; end  %play speed (frames per second)
    ...
    %initialize figure
    fig_handle = figure('Color',[.3 .3 .3],... 'MenuBar','none',...
        'Units','norm', ...
        'WindowButtonDownFcn',@button_down, 'WindowButtonUpFcn',@button_up, ...
        'WindowButtonMotionFcn', @on_click, 'KeyPressFcn', @key_press, varargin{:});
    
    %axes for text for frame number
    axes_text_handle = axes('Position',[0 0 1 1],'Visible','off');
    text_x = 0.97;
    text_y = 0.06;
    text(text_x,text_y,sprintf('Frame %d of %d', f, num_frames),...
    ...
    axes_handle = axes('Position',[0 0.15 1 0.85]);
    ...
    function play(period)
        ...
        if strcmp(get(play_timer,'Running'), 'off')
            if f == num_frames
                % Loop back if "play" is started on the last frame
                f = 1;
            end
            % Suppress warning
            prev_warn = warning('query', 'MATLAB:TIMER:RATEPRECISION');
            warning('off', 'MATLAB:TIMER:RATEPRECISION')
            set(play_timer, 'Period', period);
            warning(prev_warn.state, prev_warn.identifier)
            start(play_timer);
        else
            stop(play_timer);
        end
